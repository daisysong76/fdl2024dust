...
if __name__ == "__main__":
    ...
    path_label_train = '/home/daisysong/2024-HL-Virtual-Dosimeter/src/notebooks/train_label.pkl'#Crate
...
#    # Compute the min and max of the training labels # we will try which one is better later
#     min_label = label_train.min(dim=0)[0]
#     max_label = label_train.max(dim=0)[0]

#     # Normalize the labels to [0, 1]
#     label_train_norm = (label_train - min_label) / (max_label - min_label) # should I normalize them based on stardard deviation?
#     label_val_norm = (label_val - min_label) / (max_label - min_label)
    ...
    # Compute the mean and standard deviation of the training labels # we will try which one is better later
    mean_label = label_train.mean(dim=0)
    std_label = label_train.std(dim=0)

    # Standardize the labels to have mean 0 and standard deviation 1
    label_train_std = (label_train - mean_label) / std_label
    label_val_std = (label_val - mean_label) / std_label

    train_dataset = TensorDataset(video_train, label_train_std)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4) # overfit to these local patterns.
    val_dataset = TensorDataset(video_val, label_val_std)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=4)
