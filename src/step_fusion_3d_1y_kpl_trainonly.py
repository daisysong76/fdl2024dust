...
def main():
    ...
    # Training settings
    batch_size = 4  # TODO Reduced batch size to fit into memory
    num_epochs = 100
    lr = 1e-4  # Reduced learning rate
    seed = 93728645

    # Model
    num_frames = 7200  # TODO Number of frames (time points) in each video clip 30*24*4*12
    ...
    logging.basicConfig(filename='./logs/step_3d_1y.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    ...
    torch.save(model.state_dict(), os.path.join(outpath, 'final_3d_1y.pth'))
    np.save(os.path.join(outpath, 'val_loss_1y.npy'), np.array(test_loss_list))
    np.save(os.path.join(outpath, 'val_mse_1y.npy'), np.array(test_mse_list))
    np.save(os.path.join(outpath, 'val_rmse_1y.npy'), np.array(test_rmse_list))
