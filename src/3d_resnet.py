...
if __name__ == "__main__":
   ...
   # Compute the min and max of the training labels
    ...
    # Outputs
    outpath = './saved_models/step_3d_resnet/'  # Where to save models
    logpath = './logs/'
    ...
    logging.info('----- Define 3D ResNet -----')
    print('----- Define 3D ResNet -----')

    # Define the 3D ResNet model
    model = models.video_r3d_18(pretrained=True)  # Use a pre-trained 3D ResNet model
    model.fc = nn.Linear(model.fc.in_features, 12)  # Adjust the final layer to match the number of output classes
