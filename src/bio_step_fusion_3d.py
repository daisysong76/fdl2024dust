...
import sys
sys.path.append('/home/daisysong/2024-hl-radiation-ml/scripts')

from datasets import SDOMLlite, BioSentinel, Sequences, TarRandomAccess, WebDataset
from models import SDOSequence
from run import train, evaluate, evaluate_test_metrics

# Assuming you have defined the SDOMLlite and BioSentinel classes as shown previously

# Initialize the SDOMLlite dataset
data_files = '/home/daisysong/2024-HL-Virtual-Dosimeter/data/google_cloud_buckets/sdoml-lite-biosentinel'
channels = ['hmi_m', 'aia_0131', 'aia_0171', 'aia_0193', 'aia_0211', 'aia_1600']
sdoml_dataset = SDOMLlite(data_dir, channels=channels, date_start='2022-01-01', date_end='2022-12-31')

# Initialize the BioSentinel dataset
label_files  = 'path_to_your_biosentinel_data.csv'
biosentinel_dataset = BioSentinel(data_file, date_start='2022-11-16T11:00:00', date_end='2024-05-14T19:30:00', normalize=True)
...
dataset = NPYDataset(data_files, label_files)
...
class SwinTransformer3DFineTuner:
    def __init__(self, model, fine_tune_first_layer=True, fine_tune_last_layer=True):
        ...
        # Add normalization layer.  # TODO: change to 
...
if __name__ == "__main__":
    ...
    # Print a small portion of the tensor
    print(f"Sample values from video_train:\n{video_train[:1, :, :, :5, :5]}")
   ...
   # Compute the min and max of the training labels
    ...
    model = SwinTransformer3D(num_classes=1, # regression
                              embed_dim=96, # TODO 512
                              mlp_ratio=4.0, # hidden layer dimension = mlp_ratio * embed_dim
                              patch_norm=True, # whether to apply layer normalization on the patch embedding
                              patch_size=(2,4,4,),# split your image into small patches
                              drop_path_rate=0.001,  # stochastic depth rate in the paper
                              num_heads=[4,8,16,32],# number of heads in different layers
                              pretrained='https://download.openmmlab.com/mmaction/v1.0/recognition/swin/swin_base_patch4_window7_512.pth',
                              pretrained2d=True,# whether to load pretrained 2D model
                              window_size=(8,7,7,))
    logging.info('----- Define 3D swin -----')
...
# Ensure that any future uses of torch.load in your script or environment use weights_only=True
torch.load = lambda f: torch.load(f, weights_only=True)
